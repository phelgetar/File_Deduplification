...
### 🚦 Service status
- ✨ **NEW**: **The Run screen opens with the state of every local service.** MySQL, Ollama, the usaccidents backend and frontend, the Epstein server, the jarheads dashboard and scheduler, doc-classifier and the workbench itself — green dot for up, red triangle for down, refreshed every ten seconds. When a scan produces nothing useful the cause is often a service that quietly died, and the answer was a terminal away.
- **MySQL and Ollama are marked `NEEDED`** when down, because the workbench depends on them: a scan fails without the first and silently skips the local classification tier without the second. The other seven are informational.
- The list is **read from `start-services.sh`**, not copied into the workbench. That script already defines each service's port, label and URL, and a second copy is a copy that drifts — `classify/extract.py` sat a full minor version behind its original for exactly that reason. `WORKBENCH_SERVICES_SCRIPT` overrides the path; a missing or unparseable script leaves the panel empty rather than raising.
- What is *not* borrowed is the checking. `start-services.sh status` takes **~770 ms** and spawns nine `lsof` processes, which is fine to type and far too slow for a panel on a timer; a concurrent TCP connect to each loopback port answers in **~50 ms**. Verified to agree with the script's own answer on all 9 services, with no disagreements.
- The dot carries a **shape as well as a colour**, so the state survives a grayscale screenshot and a red/green colour-blind reader. Polling pauses while the tab is hidden.
- 🐛 A false positive found while building it: the one service with no port was matched with `pgrep -f`, which matches whole command lines — so any process merely *mentioning* the pattern counted, including the checker's own shell and its pipeline siblings, and no amount of pid filtering excluded them. It now requires the process's **working directory** to be the service's, which is how `start-services.sh` decides it too.

### 🎬 Video
- ✨ **NEW**: **The workbench can read a video.** `classify/video.py` turns a clip into searchable text — `ffprobe` for duration, resolution, codec, recording date, camera and GPS, then one frame from the midpoint, captioned and OCR'd by the same local vision pipeline images already use. Verified on a real `.mov`: 757 characters in 4.4 s, including duration, recording date, resolution, codec and a caption. `.mov .mp4 .m4v .avi .mkv .webm .mpg .mpeg .wmv`.
- 🐛 **FIX**: The merge from `doc-classifier` had gone stale. `classify/extract.py` was pinned at v1.0.0 with a comment saying it was kept byte-identical "so that fixes can still be diffed against the original" — but the original reached **v1.1.0 with video support on 2026-08-30**, so the workbench could file a `.mp4` correctly and knew nothing about what was in it. Both copies are now level at v1.1.0, and the comment says what actually happened.
- 🔧 **CHANGED**: The binaries are found on `PATH` rather than hardcoded to `/opt/homebrew`, which was right on this machine and wrong on any Intel Mac, any Linux box, and any cron job with a trimmed `PATH`. Override with `WORKBENCH_FFPROBE` / `WORKBENCH_FFMPEG`. `video.available()` reports a missing ffmpeg once instead of failing per file.
- ⚠️ **Reached today only by the cloud tier**, which is the one path that reads file contents. A `.mp4` is classified as video by its extension and never needs escalating, so a normal scan still files it without describing it. Broad video indexing arrives with the search work, which calls `extract_text` over everything. Documented in both READMEs rather than left to be discovered.
- Needs `brew install ffmpeg`. Optional: without it a video is still classified and filed, and only its contents go undescribed.

### 🗂️ Run history
- 🐛 **FIX**: **The Jobs tab only ever showed jobs from before the server started.** The registry rebuilt itself from `.workbench/jobs/` at startup and never looked again, so a command-line run — or an import — that happened while the server was up stayed invisible until a restart. A server left running for ten days listed **13 jobs against 54 on disk**, which is exactly the "I lost all my runs" problem the persistence work was meant to end. The listing now picks up directories that appeared since the last look, guarded by the jobs directory's own mtime so the unchanged case is a single `stat` — 50 listings of 54 jobs cost 2 ms. A job is also reachable by id without a restart, so a link to a plan works the moment the run finishes.
- 📖 The panel was headed "Jobs this session", which undersold it and described the bug rather than the intent. It now reads **Run history**.
